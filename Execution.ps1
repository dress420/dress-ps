...
function Get-USNJournalEntries {
    ...
    try {
        # Include all filesystem drives (including C:)
                ...
                foreach ($file in $recentFiles) {
                    if ($file.FullName -match "\.(exe|dll|scr|bat|cmd|ps1|vbs)$") {
                        $result = [PSCustomObject]@{
                            ...
                            FileExists = $true
                            Signature = (if (Test-Path $file.FullName) { Get-DigitalSignature -FilePath $file.FullName } else { "N/A" })
    ...
    try {
        ...
        foreach ($pf in $prefetchFiles) {
            try {
                ...
                # include all drives (including C:)
                $drives = Get-PSDrive -PSProvider FileSystem | Where-Object { $_.Used -gt 0 }
                foreach ($drive in $drives) {
                    $potentialPaths = @(
                        "$($drive.Root)$exeName",
...
function Show-ResultsGUI {
    ...
    $form.Text = "Executed Files & USN Journal Scan Results - Made with love by lili and dress <3"
    ...
    foreach ($result in $Results) {
        ...
        if ($signature -and $signature.Length -gt 50) {
...
Write-Log "Starting executed files and USN Journal scan on all drives including C:..."
